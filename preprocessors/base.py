...
class BaseDataset:
    ...
    def load(self, text):
        _, _, _ = self.load_all_data()
        self.all_labels = list(set(self.all_labels))
        self.load_emb = True
        self.train_emb()
        text = text.strip()
        text_split = jieba.lcut(text)
        texts_labels = [(text_split, [])]
        texts_idx, labels_origin = self.t5_data(texts_labels)
        return texts_idx, labels_origin # [[word_idx...]], [[label_idx...]]
    ...
    def index_label(self, idx):
        return self.label_idx[idx]
